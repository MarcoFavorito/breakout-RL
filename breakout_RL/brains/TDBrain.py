...
class TDBrain(Brain):
    def __init__(self, n_actions, gamma=0.99, nsteps=300):
...
class QLearning(TDBrain):
    ...
    def best_action(self, state):
        Q_values = self.Q[state] if state in self.Q else np.zeros((self.n_actions,))
...
class Sarsa(TDBrain):
    ...
    def observe(self, state, action, reward, state2):
        ...
        s_ = state2
        next_Q = self.Q[s_] if s_ in self.Q else np.zeros(self.n_actions)
